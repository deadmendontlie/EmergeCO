...
Future<String> _fetchStatus(int id) async {
  ...
  final response = await http.post('http://18.222.188.253:80/get_status',
      headers: {
...
Future<void> _updategps(int id) async {
  ...
  final jsonData = json.encode(value);
  final response = await http.put('http:/18.222.188.253:80/change_report_gps',
      headers: {
        "content-type": "application/json",
        "accept": "application/json",
      },
...
Future<int> _postEmergency() async {
  ...
  return await _postReport(jsonData);
}
...
Future<void> _testRSA() async {
  final path = await _localPath;
  final publicKey = await parseKeyFromFile<RSAPublicKey>('$path/public.pem');
  final privKey = await parseKeyFromFile<RSAPrivateKey>('$path/private.pem');
